...
container=$(buildah from docker.io/library/rockylinux:8)
commit_container=$(buildah from docker.io/library/rockylinux:8)
...
buildah run --network=host $container -- bash /localize-rocky8/localize.sh
...
buildah run --network=host $container -- cp /localize-rocky8/config /root/.cargo/config
...
buildah run --network=host $commit_container -- bash /localize-rocky8/localize.sh
buildah run --network=host $commit_container -- dnf install -y NetworkManager-libnm systemd-libs xz
...
buildah run --network=host $commit_container -- dnf remove -y xz tar
buildah run --network=host $commit_container -- dnf clean all
