...
use eyre::Result;
use nm::{
    ConnectionExt, SettingConnection, SimpleConnection, SETTING_WIRED_SETTING_NAME, SettingWired
};
...
pub async fn list_connections() -> Result<NetworkResponse> {
    let client = create_client().await?;
    let nm_connecionts = client.connections().iter().filter_map(|x| {
        let mut connection = None;
        if let Some(name) = x.id() {
            if let Some(uuid) = x.uuid() {
                if let Some(interface) = x.interface_name() {
                    connection = Some(Connection::new(name.to_string(), uuid.to_string(), Some(interface.to_string()) ))
                } else {
...
pub async fn delete_connection(conn_name: String) -> Result<NetworkResponse> {
    let client = create_client().await?;
    let nm_connecionts: Vec<nm::RemoteConnection> = client.connections().into_iter().filter_map(|x| {
        let mut connection = None;
        if let Some(name) = x.id() {
            if name == conn_name {
                connection = Some(x)
