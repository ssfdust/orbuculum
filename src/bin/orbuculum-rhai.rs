...
async fn main() {
    ...
    if let Some(devices) = sort_devices(&nicrule_file, &mut devices) {
        ...
        debug_group_devices(&devices);
        println!("==================================================\n");
        let devices = insert_device_con_names(&nicrule_file, &devices).unwrap();
        debug_final_result(&devices);
    };
}

fn debug_group_devices(devices: &Vec<serde_json::Value>) {
...
fn debug_devices(devices: &Vec<serde_json::Value>) {
...
fn debug_final_result(devices: &Vec<serde_json::Value>) {
